...
# OpenWeatherMap API Key
OPENWEATHER_API_KEY = os.getenv('OPENWEATHER_API_KEY', 'dda94a895d41c6d7d050731ac0d7a4d6')
...
# Get 5-day weather forecast for given coordinates using OpenWeatherMap
def get_forecast(lat, lon):
    ...
    if "list" in data:
        return [
            {"date": item["dt_txt"], "temperature": item["main"]["temp"], "weather": item["weather"][0]["description"]}
            for item in data["list"]
        ]
    return None

# Check if rain is expected using Open-Meteo
def check_rain(lat, lon):
    rain_url = f"https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={lon}&hourly=precipitation&timezone=auto"
    response = requests.get(rain_url)
    data = response.json()
    if "hourly" in data and "precipitation" in data["hourly"]:
        precipitation = data["hourly"]["precipitation"]
        if any(p > 0 for p in precipitation[:24]):  # Check the next 24 hours
            return True
    return False
...
def rain_alert_api():
    ...
    try:
        lat, lon = get_coordinates(place)
        is_raining = check_rain(lat, lon)

        if is_raining:
            alert_message = f"Alert: Rain is expected today in {place}. Take an umbrella!"
        else:
            alert_message = f"No rain expected today in {place}. Enjoy your day!"
